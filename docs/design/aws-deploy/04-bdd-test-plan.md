...
| Security group ingress allows TCP 80 and 443 only; port 22 appears nowhere | CDK assertions | SC4's no-SSH posture |
...
| Cost estimate review: roughly $32/month total across environments, under the $40 ceiling | pricing review | SC7 |
...
| R3 | Instance replacement loses caddy_data; certificates re-issue | M2 | documented-accepted — deliberate tradeoff; the duplicate-certificate limit is noted alongside the bootstrap runbook | open |
...
| R5 | Registry push role trusts all repository refs | M2 | integration check: each deploy role trusts only its own environment, bounding the blast radius one layer down | open |
| R6 | HTTP/3 stays off (UDP 443 closed) | M2 | integration check: security group ingress asserts TCP 80 and 443 only | open |

**Totals**: 6 residuals; claims M2=3, M3=1, M4=2 → 6/6 claimed, all open.
