...
export const DOCKER_COMPOSE_GOLDEN: string = [
  ...
  "#",
  "# RELAY_SUBSCRIBE_AUTHZ is deliberately NOT part of that sync. Both files pin it",
  "# off — that one from #91 — for unrelated reasons: there it is a wire-test",
  "# fixture (three tests need the relay gate off to prove MLS fails closed on its",
  "# own), here it is the deployment decision recorded below. The two move",
  "# independently, and that file's note about what \"a real relay\" should do does",
  "# not govern this one.",
  "#",
  ...
  "      # Pinned off explicitly rather than left unset: the relay's default flips",
  "      # to ON with #72 (PR #91), so unset would start gating YrsUpdate content",
  "      # here with no diff to this branch; `=0` is off under either default. The",
  "      # old justification — subscribe-time authz deadlocks the MLS bootstrap —",
  "      # no longer holds: #72 makes a capability-less Subscribe handshake-only,",
  "      # so a joiner still receives its Welcome and only content is gated.",
  "      # Turning it on is a deliberate later step, gated on verifying the",
  "      # DEPLOYED clients register a document anchor and present a capability.",
  "      # Admission control today still lives in the Identify exchange.",
...
export const CADDYFILE_GOLDEN: string = [
...
export const DEPLOY_SH_GOLDEN: string = [
  "#!/usr/bin/env bash",
